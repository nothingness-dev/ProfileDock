...
def _is_windows_reserved_path(candidate: Path) -> bool:
    return any(
        part.split(".", 1)[0].rstrip(" ").casefold().translate(str.maketrans("¹²³", "123"))
...
def _validate_safe_member_path(member_name: str) -> Path:
    ...
    if _is_windows_reserved_path(candidate):
        raise DecompressionSecurityError(
            f"archive member contains a Windows reserved device name: {member_name}"
        )
    return candidate
...
def _existing_profile_matches_archive(profile: Profile, archive_profile: dict[str, Any]) -> bool:
    ...
    from .backup import _is_cache_file
    ...
    for root, directories, files in os.walk(data_dir, followlinks=False):
        ...
        for filename in files:
            ...
            if relative_path not in expected_paths and _is_cache_file(relative_path):
                continue
...
def restore_backup_archive(
    ...
    def _stream() -> RestoreReport:
        with tar:
            return _restore_stream(tar, archive, data_paths, overwrite)
...
def _restore_stream(
    tar: tarfile.TarFile, archive: Path, data_paths: DataPaths, overwrite: bool
) -> RestoreReport:
    try:
