...
Keys: `j`/`k` or `↑`/`↓` navigate, `Enter` runs, `Tab`/`Shift+Tab` switch panes and form fields, `Esc` steps back, `/` filters commands, `t` toggles the dark and light themes, and single letters run commands instantly (`l` list, `s` status, `d` doctor, `g` logs, `c` create, `i` show, `o` launch, `w` close, `r` rename, `e` set-engine, `b` backup, `u` restore, `x` delete, `q` quit). Mouse clicks move the selection only; `Enter` runs the highlighted command or submits the focused form.
